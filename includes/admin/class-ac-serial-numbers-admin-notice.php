...
class AC_Serial_Numbers_Admin_Notice {
	/**
	 * AC_Serial_Numbers_Admin_Notices presisting on the next request.
	 * @var array
	 * @since 1.2.0
	 */
	...
	/**
	 * AC_Serial_Numbers_Admin_Notices displayed on the current request.
	 *
	 * @var array
	 * @since 1.2.0
	 */
	...
	public static function init() {
		self::$dismissed_notices = get_user_meta( get_current_user_id(), 'ac_serial_numbers_dismissed_notices', true );
	...
	public static function output_notices() {
		$saved_notices = get_option( 'ac_serial_numbers_notices', array() );
		...
		if ( ! empty( $notices ) ) {
			...
			foreach ( $notices as $notice ) {
				...
				$notice_classes = array( 'ac_serial_numbers_notice', 'notice', 'notice-' . $notice['type'] );
			...
			if ( function_exists( 'wc_enqueue_js' ) ) {
				wc_enqueue_js( "
					jQuery( function( $ ) {
						jQuery( '.ac_serial_numbers_notice .notice-dismiss' ).on( 'click', function() {

							var data = {
								action: 'ac_serial_numbers_notice',
								notice: jQuery( this ).parent().data( 'dismiss_class' ),
								security: '" . wp_create_nonce( 'ac_serial_numbers_notice_nonce' ) . "'
							};
			...
			// Clear.
			delete_option( 'ac_serial_numbers_notices' );
		}
	}
	...
	public static function save_notices() {
		update_option( 'ac_serial_numbers_notices', self::$saved_notices );
	}
...
//		}
	...
	public static function remove_dismissible_notice( $notice_name ) {
		...
		if ( ! self::is_dismissible_notice_dismissed( $notice_name ) ) {
			...
			update_user_meta( get_current_user_id(), 'ac_serial_numbers_dismissed_notices', self::$dismissed_notices );
...
//		}
		...
		<div class="notice-right">
			<h2 class="serial-welcome-title"><?php esc_attr_e( 'Ready to sell serial numbers?', 'ac-serial-numbers' ); ?></h2>
			<p class="serial-welcome-text"><?php esc_attr_e( 'Thank you for installing TIC Serial Numbers.', 'ac-serial-numbers' ); ?>
				<br/><?php esc_attr_e( 'Let\'s get started by inserting your first serial number!', 'ac-serial-numbers' ); ?>
			</p>
			<a href="<?php echo admin_url( 'admin.php?page=ac-serial-numbers?action=add' ); ?>"
			   class="sw-welcome-button button-primary"><?php esc_attr_e( 'Let\'s go!', 'ac-serial-numbers' ); ?></a>
		</div>
		<?php
...
AC_Serial_Numbers_Admin_Notice::init();
