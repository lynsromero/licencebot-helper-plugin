...
class AC_Serial_Numbers_Admin_Screen {
	...
	public static function render_add( $action ) {
		...
		if ( ! empty( $id ) && $serial = AC_Serial_Numbers_Query::init()->from( 'serial_numbers' )->find( $id ) ) {
			$update             = true;
			$item               = wp_parse_args( get_object_vars( $serial ), $item );
			$item['serial_key'] = ac_serial_numbers_decrypt_key( $item['serial_key'] );
		}
		?>
        <div class="wrap">
            <h1 class="wp-heading-inline">
				<?php if ( $update ): ?>
					<?php _e( 'Update Serial', 'ac-serial-numbers' ) ?>
				<?php else: ?>
					<?php _e( 'Add Serial', 'ac-serial-numbers' ) ?>
				<?php endif ?>
            </h1>
			...
			if ( ! ac_serial_numbers()->is_pro_active() ) {
				echo sprintf( '<p class="ac-serial-numbers-upgrade-box" style="background-color: #fff;">%s <a href="%s" target="_blank" class="button">%s</a></p>', __( 'Checkout the full features of TIC Serial Numbers Pro.', 'ac-serial-numbers' ), 'https://tic.com.bd/plugins/woocommerce-serial-numbers-pro/?utm_source=create_serial_page&utm_medium=button&utm_campaign=ac-serial-numbers&utm_content=View%20Details', __( 'View Details', 'ac-serial-numbers' ) );
			}
			?>
                ...
                <table class="form-table">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <p class="description"><?php esc_html_e( 'Your secret number, supports multiline. Will be encrypted before it saving. eg. d555b5ae-d9a6-41cb-ae54-361427357382', 'ac-serial-numbers' ); ?></p>
                        </td>
                    </tr>

					<?php if ( ! ac_serial_numbers_software_support_disabled() ): ?>
                        <tr>
                            <th scope="row">
                    ...
                    <tr>
                        ...
                        <td>
							<?php echo sprintf( '<input name="expire_date" id="expire_date" class="regular-text ac-serial-numbers-select-date" type="text" autocomplete="off" value="%s">', $item['expire_date'] ); ?>
					...
					<?php if ( $update ): ?>
                        ...
                        <tr>
                            <th scope="row">
                                <label for="status">
									<?php esc_html_e( 'Status', 'ac-serial-numbers' ); ?>
                                </label>
                            </th>
                            <td>
                                <select id="status" name="status" class="regular-text">
									<?php foreach ( ac_serial_numbers_get_serial_number_statuses() as $key => $option ): ?>
										<?php echo sprintf( '<option value="%s" %s>%s</option>', $key, selected( $item['status'], $key, false ), $option ); ?>
									<?php endforeach; ?>
                                </select>
                                <p class="description"><?php esc_html_e( 'The status of the serial number.', 'ac-serial-numbers' ); ?></p>
                            </td>
                        </tr>
                        <!-- order -->
                        <tr>
                            <th scope="row">
                                <label for="order_id">
									<?php esc_html_e( 'Order ID', 'ac-serial-numbers' ); ?>
                                </label>
                            </th>
                            <td>
								<?php echo sprintf( '<input name="order_id" id="order_id" class="regular-text" type="number" value="%d" autocomplete="off">', $item['order_id'] ); ?>
                                <p class="description"><?php esc_html_e( 'The order to which the serial number will be assigned.', 'ac-serial-numbers' ); ?></p>
                            </td>
                        </tr>
					<?php endif; ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <p class="submit">
                                <input type="hidden" name="action" value="ac_serial_numbers_edit_serial_number">
								<?php wp_nonce_field( 'edit_serial_number' ); ?>
								<?php if ( $update ): ?>
									<?php echo sprintf( '<input type="hidden" name="id" value="%d">', $id ); ?>
									<?php submit_button( __( 'Update Serial Number', 'ac-serial-numbers' ), 'primary', 'submit', false ); ?>
								<?php else: ?>
	...
	public static function render_table() {
		require_once dirname( __DIR__ ) . '/tables/class-ac-serial-numbers-serial-numbers-list-table.php';
		$table    = new AC_Serial_Numbers_Serial_Numbers_List_Table();
        ...
        <div class="wrap">
            <h1 class="wp-heading-inline">
				<?php _e( 'Serial Numbers', 'ac-serial-numbers' ); ?>
            </h1>
            <a href="<?php echo admin_url( 'admin.php?page=ac-serial-numbers&action=add' ) ?>"
               class="add-serial-title page-title-action">
				<?php _e( 'Add New', 'ac-serial-numbers' ) ?>
            </a>
             <a href="<?php echo admin_url( 'admin.php?page=ac-serial-numbers-add-volume-license' ) ?>"
               class="add-serial-title page-title-action">
				<?php _e( 'Add Volume License', 'ac-serial-numbers' ) ?>
            </a>
            <hr class="wp-header-end">

            <form id="ac-serial-numbers-list" method="get">
				<?php
				$table->search_box( __( 'Search', 'ac-serial-numbers' ), 'search' );
				$table->views();
				$table->display();
				?>
                <input type="hidden" name="page" value="ac-serial-numbers">
            </form>
        </div>
		<?php
	}
	...
	public static function handle_bulk_actions( $doaction ) {
		if ( $doaction ) {
			...
			foreach ( $ids as $id ) { // Check the permissions on each.
				switch ( $doaction ) {
					case 'delete':
						ac_serial_numbers_delete_serial_number( $id );
						break;
					case 'activate':
						ac_serial_numbers_update_serial_number( [
							'id'         => $id,
							'order_id'   => null,
							'order_date' => null,
							'status'     => 'available',
						...
						break;
					case 'deactivate':
						ac_serial_numbers_update_serial_number_status( $id, 'inactive' );
						break;
				}
			}
