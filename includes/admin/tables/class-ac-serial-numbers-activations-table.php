...
class AC_Serial_Numbers_Activations_List_Table extends \WP_List_Table {
	/**
	...
	public function __construct() {
		parent::__construct( array(
			'singular' => __( 'Activation', 'ac-serial-numbers' ),
			'plural'   => __( 'Activations', 'ac-serial-numbers' ),
			'ajax'     => false,
		) );
	}
	...
	public function get_views() {
		...
		$url            = admin_url( 'admin.php?page=ac-serial-numbers-activations' );
		$views          = array(
			'all'      => sprintf( '<a href="%s"%s>%s</a>', remove_query_arg( 'status', $url ), $current === 'all' || $current == '' ? ' class="current"' : '', __( 'All', 'ac-serial-numbers' ) . $total_count ),
			'active'   => sprintf( '<a href="%s"%s>%s</a>', add_query_arg( 'status', 'active', $url ), $current === 'active' ? ' class="current"' : '', __( 'active', 'ac-serial-numbers' ) . $active_count ),
			'inactive' => sprintf( '<a href="%s"%s>%s</a>', add_query_arg( 'status', 'inactive', $url ), $current === 'inactive' ? ' class="current"' : '', __( 'Inactive', 'ac-serial-numbers' ) . $inactive_count ),
		);
	...
	public function get_bulk_actions() {
		$actions = array(
			...
			'activate'   => __( 'Activate', 'ac-serial-numbers' ),
			'deactivate' => __( 'Deactivate', 'ac-serial-numbers' ),
		);
	...
	function get_columns() {
		$columns = array(
			...
			'instance'        => __( 'Instance', 'ac-serial-numbers' ),
			'serial_id'       => __( 'Serial ID', 'ac-serial-numbers' ),
//			'serial_numbers'  => __( 'Serial Numbers', 'ac-serial-numbers' ),
			'platform'        => __( 'Platform', 'ac-serial-numbers' ),
			//'product'         => __( 'Product', 'ac-serial-numbers' ),
			//'order'           => __( 'Order', 'ac-serial-numbers' ),
			//'expire_date'     => __( 'Expire Date', 'ac-serial-numbers' ),
			'activation_time' => __( 'Activation time', 'ac-serial-numbers' ),
	...
	function column_instance( $item ) {
		$actions           = array();
		$base_url          = add_query_arg( array( 'id' => $item->id ), admin_url( 'admin.php?page=ac-serial-numbers-activations' ) );
		...
		$row_actions['id'] = sprintf( __( 'ID: %d', 'ac-serial-numbers' ), $item->id );

		if ( $item->active == '0' ) {
			$row_actions['activate'] = sprintf( '<a href="%1$s">%2$s</a>', $activate_url, __( 'Activate', 'ac-serial-numbers' ) );
		}
		if ( $item->active != '0' ) {
			$row_actions['inactivate'] = sprintf( '<a href="%1$s">%2$s</a>', $deactivate_url, __( 'Inactivate', 'ac-serial-numbers' ) );
		}

		$row_actions['delete'] = sprintf( '<a href="%1$s">%2$s</a>', $delete_url, __( 'Delete', 'ac-serial-numbers' ) );
