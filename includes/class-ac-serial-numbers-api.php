...
class AC_Serial_Numbers_API {

	/**
	 * AC_Serial_Numbers_API constructor.
	 */
	public function __construct() {
		add_action( 'woocommerce_api_serial-numbers-api', array( $this, 'validate_api_request' ) );
		add_action( 'ac_serial_numbers_api_action_check', array( $this, 'check_license' ) );
		add_action( 'ac_serial_numbers_api_action_activate', array( $this, 'activate_license' ) );
		add_action( 'ac_serial_numbers_api_action_deactivate', array( $this, 'deactivate_license' ) );
		add_action( 'ac_serial_numbers_api_action_version_check', array( $this, 'version_check' ) );
	}
	...
	public function validate_api_request() {
		...
		$allow_duplicate = apply_filters( 'ac_serial_numbers_allow_duplicate_serial_number', false );
		if ( $allow_duplicate && ! is_email( $email ) ) {
			$this->send_error( [
				'error' => __( 'Email is required', 'ac-serial-numbers' ),
				'code'  => 403
			] );
		}
		if ( empty( $serial_key ) ) {
			$this->send_error( [
				'error' => __( 'Serial Number is required', 'ac-serial-numbers' ),
				'code'  => 403
			] );
		}
		...
		if ( empty( $product_id ) ) {
			$this->send_error( [
				'error' => __( 'Product id is required', 'ac-serial-numbers' ),
				'code'  => 403
			] );
		}
		...
		if ( ! in_array( $post_type, array( 'product', 'product_variation' ) ) ) {
			$this->send_error( [
				'error' => __( 'Invalid product id', 'ac-serial-numbers' ),
				'code'  => 403
			] );
		}
...
//		}
		$query_where   .= $wpdb->prepare( " AND serial_key=%s", ac_serial_numbers_encrypt_key( $serial_key ) );
		...
		if ( ! $serial_number ) {
			$this->send_error( [
				'error' => __( 'Serial Number is not associated with provided product id', 'ac-serial-numbers' ),
				'code'  => 403
			] );
		}
		...
		if ( $allow_duplicate && $order->get_billing_email( 'edit' ) !== $email ) {
			$this->send_error( [
				'error' => __( 'This email address is not associated with any serial key', 'ac-serial-numbers' ),
				'code'  => 403
			] );
		}
		...
		if ( $serial_number->status !== 'sold' ) {
			$this->send_error( [
				'error' => __( 'Serial Number is not active', 'ac-serial-numbers' ),
				'code'  => 403
			] );
		}
		...
		if ( empty( $request ) || ! in_array( $request, array(
			...
			$this->send_error( [
				'error' => __( 'Invalid request type', 'ac-serial-numbers' ),
				'code'  => 403
			] );
		}

		do_action( 'ac_serial_numbers_api_action', $serial_number );
		do_action( "ac_serial_numbers_api_action_{$request}", $serial_number );
	}
	...
	public function check_license( $serial_number ) {
		$activations = AC_Serial_Numbers_Query::init()->from( 'serial_numbers_activations' )->where( [
		...
		$this->send_success( apply_filters( 'ac_serial_numbers_check_license_response', [
	...
	public function activate_license( $serial_number ) {
		...
		$activation = AC_Serial_Numbers_Query::init()->from( 'serial_numbers_activations' )->where( [
			'serial_id' => $serial_number->id,
			'active'    => '1',
			'instance'  => $instance,
			'platform'  => $platform,
		...
		if ( empty( $activation ) && $remaining < 1 ) {
			...
			$this->send_error( [
				'error'            => __( 'Activation limit reached', 'ac-serial-numbers' ),
		...
		//if not created yet
		if ( empty( $activation ) ) {
			$activation_id = ac_serial_numbers_insert_activation( [
				'serial_id' => $serial_number->id,
				'instance'  => $instance,
				'platform'  => $platform,
				'active'    => '1',
			...
			if ( is_wp_error( $activation_id ) ) {
				$this->send_error( [
					'error'       => __( 'Activation was failed', 'ac-serial-numbers' ),
					'activations' => [],
					'code'        => 403
				] );
			}
			$activation = ac_serial_numbers_get_activation( $activation_id );
		}

		//since activation count updated so get again
		$serial_number = ac_serial_numbers_get_serial_number( $serial_number->id );
		$remaining     = intval( $this->sanitize_activation_limit($serial_number->activation_limit ) ) - intval( $serial_number->activation_count );
		$response      = apply_filters( 'ac_serial_numbers_activate_license_response', array(
			'activated'        => true,
			'remaining'        => $remaining,
			'activation_limit' => intval( $this->sanitize_activation_limit($serial_number->activation_limit ) ),
			...
			'product_id'       => $serial_number->product_id,
			'product'          => get_the_title( $serial_number->product_id ),
			'message'          => sprintf( __( 'Successfully activated. %s out of %s activations remaining', 'ac-serial-numbers' ), $remaining, $this->sanitize_activation_limit($serial_number->activation_limit ) ),
			'activations'      => $this->get_activations_response( $this->get_active_activations( $serial_number->id ) ),
		) );
	...
	public function deactivate_license( $serial_number ) {
		...
		if ( empty( $instance ) ) {
			$this->send_error( [
				'error' => __( 'Instance is  missing, You must provide an instance to deactivate license', 'ac-serial-numbers' ),
				'code'  => 403
			] );
		}
		...
		if ( empty( $activation ) ) {
			$this->send_error( [
				'error' => __( 'Could not find any related instance to deactivate ', 'ac-serial-numbers' ),
				'code'  => 403
			] );
		}

		ac_serial_numbers_update_activation( [ 'id' => $activation->id, 'active' => '0' ] );

		$serial_number = ac_serial_numbers_get_serial_number( $serial_number->id );
		$remaining     = intval( $this->sanitize_activation_limit($serial_number->activation_limit ) ) - intval( $serial_number->activation_count );

		$response = apply_filters( 'ac_serial_numbers_deactivate_license_response', array(
			'deactivated'      => true,
			'remaining'        => $remaining,
			'activation_limit' => intval( $this->sanitize_activation_limit($serial_number->activation_limit ) ),
			'message'          => sprintf( __( 'Deactivation completed. %s out of %s activations remaining', 'ac-serial-numbers' ), $remaining, $this->sanitize_activation_limit($serial_number->activation_limit ) ),
		) );
	...
	public function get_active_activations( $serial_id ) {
		return AC_Serial_Numbers_Query::init()->from( 'serial_numbers_activations' )->where( [
			'serial_id' => $serial_id,
			'active'    => '1'
		] )->get();
	}
