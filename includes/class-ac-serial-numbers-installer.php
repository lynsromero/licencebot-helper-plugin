...
class AC_Serial_Numbers_Installer {
	/**
	 * Store update files
	 *
	 * @var array
	...
	public static function init() {
		//Get plugin version
		self::$current_version = get_option( 'ac_serial_numbers_version', null );
	...
	private static function can_install() {
		return ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) && ! defined( 'IFRAME_REQUEST' ) && ! 'yes' === get_transient( 'ac_serial_numbers_installing' );
	}
	...
	private static function should_install() {
		return empty( get_option( 'woocommerceserialnumbers_version' ) ) && empty( 'ac_serial_numbers_version' );
	}
	...
	public static function install() {
		...
		set_transient( 'ac_serial_numbers_installing', 'yes', 10 );

		// Create tables.
		self::create_tables();
		$settings = array(
			'ac_serial_numbers_autocomplete_order'        => 'yes',
			'ac_serial_numbers_reuse_serial_number'       => 'no',
			'ac_serial_numbers_disable_software_support'  => 'no',
			'ac_serial_numbers_enable_stock_notification' => 'yes',
			'ac_serial_numbers_hide_serial_number'        => 'yes',
			'ac_serial_numbers_revoke_status_failed'      => 'yes',
			'ac_serial_numbers_revoke_status_refunded'    => 'yes',
			'ac_serial_numbers_revoke_status_cancelled'   => 'yes',
			'ac_serial_numbers_stock_threshold'           => '5',
			'ac_serial_numbers_notification_recipient'    => get_option( 'admin_email' ),
		);
		...
		if ( false === wp_next_scheduled( 'ac_serial_numbers_hourly_event' ) ) {
			wp_schedule_event( time(), 'hourly', 'ac_serial_numbers_hourly_event' );
		}

		if ( false === wp_next_scheduled( 'ac_serial_numbers_daily_event' ) ) {
	...
	private static function update_version() {
		delete_option( 'ac_serial_numbers_version' );
		add_option( 'ac_serial_numbers_version', ac_serial_numbers()->get_version() );
	}
	...
	public static function plugin_row_meta( $links, $file ) {
		...
		if ( $file == ac_serial_numbers()->plugin_basename() ) {
			$upgrade_link = 'https://tic.com.bd/plugins/woocommerce-serial-numbers-pro';
			$row_meta     = array(
				'docs'    => '<a href="https://tic.com.bd/docs/woocommerce-serial-numbers/">' . __( 'Documentation', 'ac-serial-numbers' ) . '</a>',
				'upgrade' => '<a href="' . esc_url( $upgrade_link ) . '" style="color: red;font-weight: bold;" target="_blank">' . __( 'Upgrade to PRO', 'ac-serial-numbers' ) . '</a>',
			);
	...
	public static function maybe_update() {
		...
		if ( version_compare( $installed_version, ac_serial_numbers()->get_version(), '<' ) ) {
			...
			delete_option( $key );
			update_option( $key, ac_serial_numbers()->get_version() );
			update_option( 'ac_serial_numbers_version', ac_serial_numbers()->get_version() );
		}
	...
	public static function custom_cron_schedules( $schedules ) {
		$schedules ['once_a_minute'] = array(
			'interval' => 60,
			'display'  => __( 'Once a Minute', 'ac-serial-numbers' )
		);
	...
	public static function action_links( $links ) {
		$links['settings'] = sprintf( '<a href="%s">', admin_url( 'admin.php?page=ac-serial-numbers-settings' ) ) . __( 'Settings', 'ac-serial-numbers' ) . '</a>';
...
AC_Serial_Numbers_Installer::init();
