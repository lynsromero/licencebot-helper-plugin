...
class AC_Serial_Numbers_Handler {
	...
	public static function validate_checkout() {
		...
		foreach ( $car_products as $id => $cart_product ) {
			...
			$allow_backorder = apply_filters( 'ac_serial_numbers_allow_backorder', false, $product_id );
			if ( ac_serial_numbers_product_serial_enabled( $product_id ) && ! $allow_backorder ) {
				...
				$source            = apply_filters( 'ac_serial_numbers_product_serial_source', 'custom_source', $product_id, $needed_quantity );
				if ( 'custom_source' == $source ) {
					$total_number = AC_Serial_Numbers_Query::init()
					                                       ->from( 'serial_numbers' )
					...
					if ( $total_number < $needed_quantity && ! ac_serial_numbers_product_serial_source_type( $product_id ) ) {
						$stock   = floor( $total_number / $per_item_quantity );
						$message = sprintf( __( 'Sorry, There is not enough serial numbers available for %s, Please remove this item or lower the quantity, For now we have %s Serial Number for this product.', 'ac-serial-numbers' ), '{product_title}', '{stock_quantity}' );
						$notice  = apply_filters( 'ac_serial_numbers_low_stock_message', $message );
	...
	public static function maybe_autocomplete_order() {
		if ( is_checkout() && ! empty( is_wc_endpoint_url( 'order-received' ) ) && ! empty( get_query_var( 'order-received' ) ) ) {
			...
			if ( ! ac_serial_numbers_validate_boolean( get_option( 'ac_serial_numbers_autocomplete_order' ) ) ) {
				return;
			}
			...
			if (! self::get_order_serial_status( $order_id ) ) {
				return $order;
			}
			...
			if ( in_array( $order->get_status(), apply_filters( 'ac_serial_numbers_autocomplete_statuses', [ 'processing' ] ), true ) ) {
				$order->update_status( 'completed' );

				$order->add_order_note( __( 'Order marked as complete by WC Serial Numbers', 'ac-serial-numbers' ) );
			}
	...
	public static function maybe_assign_serial_numbers( $order_id ) {
		$manual_delivery = apply_filters( 'ac_serial_numbers_maybe_manual_delivery', false );
		$order           = wc_get_order( $order_id );
		$order->add_order_note( $order->get_status() );
		if ( ! $manual_delivery ) {
			ac_serial_numbers_order_connect_serial_numbers( $order_id );
		}
	}
	...
	public static function revoke_serial_numbers( $order_id ) {
		...
		return ac_serial_numbers_order_disconnect_serial_numbers( $order_id );
	}
	...
	public static function order_print_items( $order ) {
		if ( ac_serial_numbers_order_has_serial_numbers( $order ) ) {
			ac_serial_numbers_get_order_table( $order );
		}
	}
...
AC_Serial_Numbers_Handler::init();
