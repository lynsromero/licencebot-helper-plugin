...
class AC_Serial_Numbers_Query {
	 ...
	 *
	 * @return AC_Serial_Numbers_Query this for chaining.
	 * @global object $wpdb
	 *
	 * @since 1.0.0
	 *
	...
	public function clear_transients($transients = null) {
		global $wpdb;
		if (!empty($transients) && is_array($transients)) {

			// Add '_transient_' prefix to each transient name
			$transients = array_map(function($transient) {
				return '_transient_' . $transient;
			}, $transients);
			
			$placeholders = implode(',', array_fill(0, count($transients), '%s'));
			$query = "DELETE FROM {$wpdb->options} WHERE option_name IN ($placeholders)";
			$result = $wpdb->query($wpdb->prepare($query, ...$transients));
	
			if ($result) {
				return "Successfully deleted {$result} transient(s).";
			} else {
				return "No transients were deleted.";
			}
		}
		
		return "No transients provided.";
	}
	 ...
	 *
	 * @return AC_Serial_Numbers_Query The current query builder.
	 */
	public function where( $column, $param1 = null, $param2 = null, $joint = 'and' ) {
		...
		if ( is_array( $column ) ) {
			...
			$subquery = new AC_Serial_Numbers_Query();
	 ...
	 *
	 * @return AC_Serial_Numbers_Query
	 * @since 1.0.0
	 */
